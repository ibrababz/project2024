...
@keras_cv_export("keras_cv.layers.RandomRotationHMap")
class RandomRotationHMap(VectorizedBaseImageAugmentationLayer):
    ...
    def augment_segmentation_masks(
        ...
        if self.segmentation_classes:
            ...
            rotated_mask = self._rotate_masks(
    ...
    def _rotate_masks(self, images, transformations):
        images = preprocessing_utils.ensure_tensor(images, self.compute_dtype)
        original_shape = images.shape
        image_shape = tf.shape(images)
        img_hd = tf.cast(image_shape[H_AXIS], tf.float32)
