...
class RandomRotationHmap2(BaseImagePreprocessingLayer):
    ...
    **Note:** This layer is safe to use inside a `tf.data` pipeline
    ...
    def transform_segmentation_masks(self, segmentation_masks, transformation, training=True):
        if training:
            if transformation["rotation_matrix_masks"] is not None:
                return self.backend.image.affine_transform(
                    images=segmentation_masks,
                    transform=transformation["rotation_matrix_masks"],
                    interpolation=self.interpolation,
                    fill_mode=self.fill_mode,
                    fill_value=0.0,
                    data_format=self.data_format,
    ...
    def get_random_transformation(self, data, training=True, seed=None):
        ...
        if masks is not None:      
            mask_shape = ops.core.shape(masks)
        ...
        if masks is not None:
            rotation_matrix_masks = self._compute_affine_matrix(
                center_x=center_x,
                center_y=center_y,
                angle=angle,
                translate_x=ops.numpy.zeros([batch_size]),
        ...
        if masks is not None:
            if len(mask_shape) == 3:
                rotation_matrix_masks = self.backend.numpy.squeeze(
                    rotation_matrix_masks, axis=0
                )
        ...
        return {
            ...
            "rotation_matrix_masks": rotation_matrix_masks,
