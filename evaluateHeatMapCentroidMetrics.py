...
if __name__ =='__main__':
    ...
    wLoadFolder, wLoadSubFolder = 'project2024', os.path.join('resnet_test_13_real_8bit_res2_ep_(0, 1000)_lr_1e-04', 'test_output_sifted')
    wLoadDir= os.path.join(ROOT_DIR, wLoadFolder, wLoadSubFolder)
    wScale = 20
    wBnMetrics = BinaryMetricsManager(wLoadDir)
    wBnMetrics.loadData()

    for i in range(wBnMetrics.getDataSize()):
        wIm = wBnMetrics.getImage(i)
        # show_wait(wIm, wScale)
        wCentList = wBnMetrics.computeCentroidsOnIm(wIm, iMinPercentArea = 0.0)
        wImClr = wBnMetrics.getClrImage(i)
        wBnMetrics.drawCentroidsOnIm(wImClr, wCentList, iRadius=1)
