...
# just for orignal image objects
def save_original_image_names(save_dir, XY_data, subset_name, label='1'):
...
def ComputeLosses(X_train, map_lists, model, modelFlag, dim, image_processor, batch_loss_tracker, batch_acc_tracker, i, j, seq=None, train=True, iNorm=0):
    ...
    if modelFlag == 'swin':
        X_train_aug = image_processor(np.array(X_train_aug), return_tensors="tf")[
            'pixel_values']
    elif modelFlag == 'resnet':
        X_train_aug = image_processor(
            np.array(X_train_aug, dtype=np.float32)[..., ::-1])
    ...
    dim_grid1, dim_grid2 = (
        dim_grid1[0], dim_grid1[1]), (dim_grid2[0], dim_grid2[1])
    # print('dim_grid1 = ', dim_grid1, 'dim_grid2 = ', dim_grid2)

    map_aug_list1, weight_list1, _ = ProcessMapList3D(
        map_lists[0], dim, dim_grid1, copyTemp(temp))
    map_aug_list2, weight_list2, _ = ProcessMapList3D(
        map_lists[1], dim, dim_grid2,  copyTemp(temp))

    act_aug_list1 = act_list_3D(map_aug_list1)
    act_aug_list2 = act_list_3D(map_aug_list2)

    pos1, neg1 = tensor_pos_neg_loss(
        y_pred0[..., 0], map_aug_list1, act_aug_list1)
    pos2, neg2 = tensor_pos_neg_loss(
        y_pred1[..., 0], map_aug_list2, act_aug_list2)
    ...
    if j == 5 and i % 15 == 0:
        ...
        if i % 2 == 0:
            map_list1, _, _ = ProcessMapList3D(
                map_lists[0], dim, dim_grid1, None)
            show_batch(flat_map_list_v2(map_list1))
        else:
            map_list2, _, _ = ProcessMapList3D(
                map_lists[1], dim, dim_grid2, None)
    ...
    for batch in batch_generator_01:
        ...
        if train:
            ...
            with tf.GradientTape() as tape:
                ...
                loss1_update, loss2_update, loss_value, acc_value = ComputeLosses(
                    X_train, map_lists, model, modelFlag, dim, image_processor, batch_loss_tracker, batch_acc_tracker, i, j, seq, train, iNorm)

            grads = tape.gradient(
                [loss1_update, loss2_update], model.trainable_weights)
            opt.apply_gradients((grad, var) for (grad, var) in zip(
                grads, model.trainable_weights) if grad is not None)
...
def check_val_loss(min_val_loss_counter, valid_loss_per_epoch, min_val_loss, old_name, epoch):
    if(valid_loss_per_epoch < min_val_loss):
        ...
        current_name = save_and_print(
            epoch, model, save_dir, add_flag='_min_val')
...
def addIndexToIndexList(iIndexList, iIndex):
    oIndexList = []
    for i in range(len(iIndexList)):
        oIndexList.append(iIndexList[i])
        if iIndex > -iIndexList[i] and iIndex < -iIndexList[i+1]:
            oIndexList.append(-iIndex)
    return oIndexList
...
def TrainAndTestV2(XY_train, XY_valid, model, modelFlag, opt, batch_size, image_processor, epochs, save_dir, unfreezeFlag=0, iNorm=0):
    ...
    if unfreezeFlag:
        ...
        wIndexList = addIndexToIndexList(wIndexList, 23) #extra steps where it explodes
        wIndexList = addIndexToIndexList(wIndexList, 24) #extra steps where it explodes

    for i in range(start, end):
        #print("\nStart of epoch %d" % (i,))
        # affine_augments() (if iNorm =0 i.e. loaded data are already floats b/w 0 and 1)
        seq = augments()
        seq.seed_(i)

        batch_gen_train = generate_batch(XY_train, batch_size, i)
        loss_per_epoch, acc_per_epoch = TrainEpochV3(
            model, modelFlag, dim, opt, batch_gen_train, image_processor, i, seq, True, iNorm)
        ...
        valid_loss_per_epoch, valid_acc_per_epoch = TrainEpochV3(
            model, modelFlag, dim, None,  batch_gen_valid, image_processor, i, None, False, iNorm)
        ...
        min_train_loss, min_train_loss_counter = check_train_loss(
            min_train_loss_counter, sum_loss_per_epoch, min_train_loss)
        min_val_loss, min_val_loss_counter, old_name = check_val_loss(
            min_val_loss_counter, sum_val_loss_per_epoch, min_val_loss, old_name, i)
        ...
        if sum_val_loss_per_epoch <= max_min_val_loss:
            ...
            min_name_list.append(save_and_print(
                i, model, save_dir, '_list_'+str(sum_val_loss_per_epoch) + wUnfrzIdx, '.hdf5', 0))
            saveAndPrintOpt(i, opt, save_dir, '_list_' +
                            str(sum_val_loss_per_epoch) + wUnfrzIdx + '_opt', '.npy', 0)
        ...
        last_n_name_list.append(save_and_print(i, model, save_dir, '_last_'+str(
            last_n) + '_'+str(sum_val_loss_per_epoch) + wUnfrzIdx, '.hdf5', 0))
        saveAndPrintOpt(i, opt, save_dir, '_last_'+str(last_n) + '_' +
                        str(sum_val_loss_per_epoch) + wUnfrzIdx+'_opt', '.npy', 0)
        ...
        if (i-start) > 0 and (i-start) % 25 == 0:
            save_and_print(i, model, save_dir, '_savept_' +
                           str(sum_val_loss_per_epoch) + wUnfrzIdx, '.hdf5', 0)
            saveAndPrintOpt(i, opt, save_dir, '_savept_' +
                            str(sum_val_loss_per_epoch) + wUnfrzIdx + '_opt', '.npy', 0)
        ...
        print('tr L:', np.round(loss_per_epoch, 3), 'A:', np.round(acc_per_epoch, 3), 'val L:', np.round(
            valid_loss_per_epoch, 3), 'A:', np.round(valid_acc_per_epoch, 3), 'ep:', i, 'min_V:', np.round(min_val_loss, 3))
        ...
        if unfreezeFlag:
            if (i-start) % 5 == 0 and (i-start) >= 400 and len(wIndexList) > 1:
                ...
                idx2 = wIndexList[0]
                print(
                    "setting model.layers[%s:%s] to trainable" % (idx2, idx1))
...
def save_and_print(i, model, save_dir, add_flag='', iFileFormat='.hdf5', verbose=1):
    ...
    full_name = os.path.join(save_dir, 'model_epoch_' +
                             number+add_flag+iFileFormat)
...
# %%
    iNorm = 255. #if 8bit set iNorm to 255.
    iSrcDir = "data2019\\1\\train_synth_8bit_224_224_sample_4"
...
# %%
    iValidSrcDir = "data2019\\1\\valid_synth_8bit_224_224_sample_4"
...
# %%
    ...
    base_model2 = tf.keras.models.Model([base_model.inputs], [
                                        base_model.output, base_model.layers[index].output], name='base_model2')
...
# %%
    ...
    # model.layers[-1].summary()
    # load_folder ="resnet_t3000_v900_fullcycle_test4_ep_(0, 1200)_lr_1e-04"
    # #load_folder = "resnet_t3000_v900_light_ep_(0, 400)_lr_1e-04"
    # load_dir = os.path.join(os.getcwd(), load_folder)
    # model.load_weights(os.path.join(load_dir,"model_epoch_1199_last_5_2.288954_Unfrz_-176.hdf5" ))
...
# %%
    ...
    model_name = "resnet_t3000_v900_fullcycle_8bit_"+str(W)+"_"+str(H)+"_sample_4_test1"
    batch_size = 8
    epochs = (0, 1210)

    lr = 0.0001  # WHEN FINE-TUNING, LOWER LEARNING RATE
    opt_dict = {'Adam': tf.keras.optimizers.Adam(learning_rate=lr), 'Adam_clip': tf.keras.optimizers.Adam(
        lr=lr, clipnorm=0.5), 'sgd': tf.keras.optimizers.SGD(lr=lr, decay=0.75*lr, momentum=0.9, nesterov=True)}
    opt_index = 'Adam'
    opt = opt_dict[opt_index]
    print('optimizer = ', opt_index, ' ||  lr = ', lr)

    save_folder = model_name + '_ep_' + \
        str(epochs)+"_lr_" + "{:.0e}".format(lr)
    save_dir = os.path.join(os.getcwd(), save_folder)
    if not os.path.exists(save_dir):
        os.mkdir(save_dir)
    plot_model(model.layers[-1], os.path.join(save_dir,
               'top_model.png'), show_shapes=True)
...
# %%
    ...
    # unfreezeFlag = 0 # 1
# %%
    train_loss_tracker, test_loss_tracker = TrainAndTestV2(
        wDataObjectList, wValidDataObjectList, model, modelFlag, opt, batch_size, image_processor, epochs, save_dir, unfreezeFlag, iNorm)
    ...
    with open(os.path.join(save_dir, 'train_test_values.csv'), 'w', newline='') as file:
        ...
        for tr1, tr2, t1, t2 in zip(train_loss_tracker[0], train_loss_tracker[1],  test_loss_tracker[0], test_loss_tracker[1]):
            ...
            if t_tot < min_test_loss:
                min_test_loss = t_tot
            row = ['train L: ', tr1, ' |A: ', tr2,  ' ||test L: ', t1, ' |A: ',
                   t2,  ' ||epoch: ', epoch_no, ' ||min_t_L: ', min_test_loss]
            writer.writerow(row)
            epoch_no += 1
# %%
...
# %%
    ...
    for batch in wTestBatchGen:
        ...
        if modelFlag == 'swin':
            X_train = image_processor(np.array(X_train), return_tensors="tf")[
                'pixel_values']
        elif modelFlag == 'resnet':
            X_train = image_processor(
                np.array(X_train, dtype=np.float32)[..., ::-1])
