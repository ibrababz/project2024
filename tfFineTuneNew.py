...
gDefaultTrainSubDir = os.path.join('data4K', 'train_real_448_res2')
...
parser.add_argument('mModelCkpt', type=Path,
                    help='Path to directory for model to fine tune')
...
if __name__ == '__main__':
    ...
    wCheckpoint.read(wModelCkpt)
    wCkptName=str(wModelCkpt).split('cp-')[-1].split('.')[0]
    print(f"\nLoaded model cp-{wCkptName}\n")
    ...
    wCallBacks.append(CustomLearningRateScheduler(wLRSchedule, iBegin = wLRBegin, iPatience = wLRPatience)) #Set to Patience to 0 to deactivate auto LR decrease
