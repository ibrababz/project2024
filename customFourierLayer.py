...
class FourierMixHMap(BaseImageAugmentationLayer):
    ...
    def _apply_fftfreq(self, h, w):
        ...
        fx = FourierMixHMap._fftfreq(w)[: w // 2 + 1 + w % 2]
        fy = FourierMixHMap._fftfreq(h)
    ...
    def _batch_augment(self, inputs):
        ...
        inputs["images"] = tf.cast(images, dtype=tf.uint8)
        return inputs
    ...
    def _update_segmentation_masks(
        ...
        segmentation_mask_size = segmentation_masks.shape[-3:-1]
        masks = tf.image.resize(masks, segmentation_mask_size, ResizeMethod.NEAREST_NEIGHBOR)
        segmentation_masks = (
