...
class RandomTranslationHmap2(BaseImagePreprocessingLayer):
    ...
    **Note:** This layer is safe to use inside a `tf.data` pipeline
    ...
    def get_random_transformation(self, data, training=True, seed=None):
        ...
        if masks is not None:      
            mask_shape = self.backend.shape(masks)
            ...
            if len(mask_shape) == 4:
                ...
                if self.data_format == "channels_last":
                    ...
                    mask_height = mask_shape[2]
                    mask_width = mask_shape[3]
            else:
                batch_size = 1
                if self.data_format == "channels_last":
        ...
        if masks is not None:
            height_translate_mask = self.backend.numpy.multiply(height_translate, mask_height)
        ...
        if masks is not None:
            width_translate_mask = self.backend.numpy.multiply(width_translate, mask_width)
        ...
        if masks is not None:
            mask_translations = self.backend.cast(
                self.backend.numpy.concatenate(
                    [width_translate_mask, height_translate_mask], axis=1
                ),
                dtype="float32",
            )
        else:
            mask_translations = None
        
        
        
        return {"translations": translations, "mask_translations": mask_translations, "input_shape": images_shape}
    ...
    def _translate_masks(self, inputs, transformation):
        if transformation is None:
            return inputs
        ...
        translations = transformation["mask_translations"]
