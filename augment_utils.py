...
def augmentWrapper(iSeqDeepCopy, kwargs):
    return iSeqDeepCopy(*kwargs)

def augmentXData(X_train, seq):
    if seq is not None:
        # wPool=mp.dummy.Pool(16)
        # kwargs = [{'images':wX} for wX in X_train]
        return seq.deepcopy()(images = X_train)
        # return wPool.map(augmentWrapper(seq.deepcopy()), kwargs)
    else:
...
#   random_order=True
#   )
#   seq = iaa.Sequential([flipX, flipY, random_trns]) #REMOVED random_trns
#   return seq

# def chooseAugments(iInt):
#     #return function definition not instance
#     if iInt==1:
#         return affineFlipDistortion
#     elif iInt==2:
#         return augments
#     else:
#         return